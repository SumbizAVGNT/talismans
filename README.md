...
## Добавление своей текстуры тотема

1. Создайте собственный ресурс-пак или используйте существующий.
2. Добавьте новую модель `item/totem_<id>.json` c `parent: "minecraft:item/generated"` и `textures: { layer0: "<namespace>/item/totem_<id>" }`.
3. Поместите PNG в `assets/<namespace>/textures/item/totem_<id>.png`.
4. В `config.yml` задайте `resource.model_path: item/totem_<id>` и `resource.material: TOTEM_OF_UNDYING`.
5. Для Nexo можно использовать `resource.from_nexo: <nexo-id>` — текстура возьмётся из Nexo без отдельного ресурспака.

## Админ-GUI

- Команда: `/talismans gui [страница]` (только `talismans.admin`).
- Возможности:
  - Листание всех талисманов/сфер с постраничной навигацией.
  - ЛКМ — выдать предмет, ПКМ — сменить «реликвию» крафта (NETHER_STAR/TOTEM/DRAGON_BREATH/HEART_OF_THE_SEA).
  - Shift+ЛКМ — включить/выключить быстрый крафт; клавиша `Q` по предмету — переключить shaped/shapeless.
  - Кнопка «Создать новый» зарезервирована под будущий live-конструктор (пока правьте `config.yml`).

## Новые анархичные механики

Добавьте ключ `anarchy_mechanics` в описание талисмана/сферы:
...
| Ключ | Эффект |
|------|--------|
| `chaos_shield` | На входящий урон: режет ~35% урона, выдаёт Absorption, кулдаун ~12 c. |
| `blood_link` | На атаку: лечит ~18% нанесённого урона. |
| `void_step` | На приседание: телепорт вперёд с кулдауном. |
| `relic_guard` | Спасает от смерти, тратя реликвию (Nether Star/Тотем) из инвентаря. |
| `meteor_call` | Шанс вызвать мини-метеор (fireball) по цели при ударе. |
| `overcharge` | При смене слота — краткий бафф скорости и силы. |
...
| `/talismans gui [page]` | Админ-GUI со всеми сферами/талисманами |

## Разрешения

| Разрешение | Описание |
|------------|----------|
| `monntalismans.admin` | Доступ ко всем командам |
| `monntalismans.give` | Выдача предметов |
| `monntalismans.reload` | Перезагрузка |
...
MonnTalismans включает уникальные механики, которых нет в других плагинах талисманов.
